...
pub fn signup_form() -> Html {
    ...
    let render_trigger = use_state(|| ()); // New hook to force re-renders
                ...
                match response_result {
                    Ok(response) => {
                        ...
                        if response.ok() {
                            ...
                            *message.borrow_mut() = "Signup failed X".to_string();
                            log::info!("Response Text: {:?}", response_text);
                        } else if response.status() == 409 {
                            *message.borrow_mut() = "Signup failed Y".to_string();
                            log::warn!("Signup failed due to a duplicate username or email");
                        } else {
                            *message.borrow_mut() = "Signup failed Z".to_string();
                            log::warn!("Request failed with status: {:?}", response.status());
                        }
                        render_trigger.set(()); // Force a re-render
                    }
                    Err(error) => {
    ...
    html! {
        <main class="home">
            ...
            <form {onsubmit} method="post" class="registration-form">
                ...
                <p class="error-text">{&*message.borrow()}</p>
