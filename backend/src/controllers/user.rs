...
pub async fn list_users(Extension(pool): Extension<PgPool>) -> Result<Json<Vec<UserModel>>, CustomError> {
...
pub async fn get_user(Path(user_id): Path<i64>, Extension(pool): Extension<PgPool>) -> Result<Json<UserModel>, CustomError> {
...
pub async fn verify_user(Extension(pool): Extension<PgPool>, Json(credentials): Json<LoginUserSchema>) -> Result<Json<UserLoginResponse>, CustomError> {
