...
impl Db {
    ...
    pub async fn delete_user_soft(user_id: i64, pool: &PgPool) -> Result<PgQueryResult, sqlx::Error> {
        sqlx::query!(
    ...
    pub async fn verify_user(credentials: LoginUserSchema, pool: &PgPool) -> Result<Option<UserModel>, sqlx::Error> {
        sqlx::query_as!(
            UserModel,
    ...
    pub async fn update_user(user_id: i64, user: UpdateUserSchema, pool: &PgPool) -> Result<UserModel, sqlx::Error> {
        sqlx::query_as!(
            UserModel,
    ...
    pub async fn create_user(user: CreateUserSchema, pool: &PgPool) -> Result<UserModel, sqlx::Error> {
        sqlx::query_as!(
            UserModel,
