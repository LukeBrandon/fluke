use crate::{
    ...
    router,
    store::{set_auth_user, set_page_loading, Store},
};
use yew::prelude::*;
          ...
          <div>
            ...
            if let Some(user) = user {
              <div class="mt-8">
                ...
                <p class="mb-4">{format!("Name: {}", user.first_name)}</p>
                <p class="mb-4">{format!("Name: {}", user.last_name)}</p>
                <p class="mb-4">{format!("Email: {}", user.email)}</p>
                <p class="mb-4">{format!("Role: {}", user.is_admin)}</p>
              </div>
            } else {
